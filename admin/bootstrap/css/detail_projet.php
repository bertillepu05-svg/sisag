...
try {
    $sql_approuved = "SELECT c.*, p.nom_projet, p.commune FROM commentaire c JOIN projet p ON c.id_projet = p.id_projet WHERE c.statut = 'approuve' AND c.id_projet = :id_projet  ORDER BY c.date_commentaire DESC LIMIT 4";
    $stmt_approuved = $conn->prepare($sql_approuved);
    $stmt_approuved->bindParam(':id_projet', $id_projet);
    $stmt_approuved->execute();
    $approuved = $stmt_approuved->fetchAll(PDO::FETCH_ASSOC);
} catch(PDOException $e) {
    $approuved = [];
}
...
<head>
    ...
    <title>Detail_projet_admin</title>
