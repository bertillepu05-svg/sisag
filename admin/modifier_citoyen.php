...
if (!isset($_GET['id'])) {
    ...
    $_SESSION['error_message'] = ("Aucun citoyen sélectionné !");
...
if ($result->num_rows === 0) {
    $_SESSION['error_message'] = ("Citoyen introuvable !");
    header("Location: modifier_adm.php");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $statut = trim($_POST["statut"]);
        ...
        if (empty($mot_de_passe)) {
            $mot_de_passe = $citoyen['mot_de_passe'];
        }

        // Utiliser prepared statements pour éviter les injections SQL
        $update = "UPDATE citoyen 
                   SET nom = ?, prenom = ?, sexe = ?, email = ?, mot_de_passe = ?, telephone = ?, commune = ?, quartier = ?, profession = ?, statut = ?
                   WHERE id_citoyen = ?";
        
        $stmt = $conn->prepare($update);
        $stmt->bind_param("ssssssssssi", $nom, $prenom, $sexe, $email, $mot_de_passe, $telephone, $commune, $quartier, $profession, $statut, $id);

        if ($stmt->execute()) {
            $_SESSION['success_message']  = "Citoyen mis à jour avec succès !";
    ...
    <div class="container-fluid px-3 px-md-4 px-lg-5">
        ...
        <div class="form-container pb-4 w-75 w-lg-50 mx-auto rounded-3 mt-4 mt-md-5" style="box-shadow: 0 2px 15px rgba(0,0,0,0.1);">
            <div class="form-header rounded-top pt-2 pb-1 ps-3 bg-dark text-white">
                <p><i class="bi bi-person-plus-fill me-2"></i>Modier un citoyen</p>
            </div>
            
            <div class="form-body px-3 px-md-4">
            <form action="modifier_citoyen.php?id=<?php echo $id; ?>" method="POST" id="inscriptionForm">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 form-group mt-3">
                            ...
                            <select name="sexe" class="form-select" required>
                                ...
                                <option value="M" <?= $citoyen['sexe'] == 'M' ? 'selected' : '' ?>>Masculin</option>
                                <option value="F" <?= $citoyen['sexe'] == 'F' ? 'selected' : '' ?>>Féminin</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 form-group mt-3">
                            ...
                            <input type="text" name="telephone" class="form-control" value="<?= htmlspecialchars($citoyen['telephone'])?>" required>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-4 form-group mt-3">
                            ...
                            <input type="text" name="commune" class="form-control" value="<?= htmlspecialchars($citoyen['commune'])?>" required>
                        </div>
                        <div class="col-md-4 form-group mt-3">
                            <label class="form-label required">Quartier</label>
                            <input type="text" name="quartier" class="form-control" value="<?= htmlspecialchars($citoyen['quartier'])?>" required>
                        </div>
                        <div class="col-md-4 form-group mt-3">
                            <label class="form-label required">Profession</label>
                            <input type="text" name="profession" class="form-control" value="<?= htmlspecialchars($citoyen['profession'])?>" required>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 form-group mt-3">
                            ...
                            <input type="text" class="form-control" 
                                value="<?= $citoyen['statut'] ?> (géré automatiquement)" readonly>
                            <input type="hidden" name="statut" value="<?= $citoyen['statut'] ?>">
                        </div>
                    </div>

                    <!-- Boutons d'action -->
                   <div class="d-flex justify-content-between mt-4">
                        <a href="gestion_citoyen.php" class="btn btn-secondary">
